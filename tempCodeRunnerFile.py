...
def agenda():
    try:
        ...
        # Query base com garantia de valores numéricos
        query = Orcamento.query.filter(
            Orcamento.data_servico >= hoje,
            Orcamento.valor_total.isnot(None),
            Orcamento.valor_mao_obra.isnot(None),
            Orcamento.valor_material.isnot(None)
        ).order_by(Orcamento.data_servico.asc())
        
        # Aplicar filtros
        if filtro == 'hoje':
            compromissos = query.filter(Orcamento.data_servico == hoje).all()
        elif filtro == 'semana':
            fim_semana = hoje + timedelta(days=7)
            compromissos = query.filter(Orcamento.data_servico <= fim_semana).all()
        elif filtro == 'mes':
            fim_mes = hoje + timedelta(days=30)
            compromissos = query.filter(Orcamento.data_servico <= fim_mes).all()
        else:
            compromissos = query.all()
            
    except Exception as e:
        app.logger.error(f'Erro ao carregar agenda: {str(e)}', exc_info=True)
        flash(f'Erro ao carregar agenda: {str(e)}', 'error')
        compromissos = []
...
def api_agendar():
    try:
        ...
        required_fields = ['cliente_nome', 'data_servico', 'servico']
        if not all(field in data and data[field] for field in required_fields):
            return jsonify({'success': False, 'message': 'Preencha todos os campos obrigatórios'}), 400
            
        # Formatação da data
        try:
            data_servico = datetime.strptime(data['data_servico'], '%Y-%m-%d').date()
        except ValueError:
            return jsonify({'success': False, 'message': 'Data inválida. Use o formato AAAA-MM-DD'}), 400
        
        # Criação do novo compromisso
        novo_compromisso = Orcamento(
        ...
        return jsonify({
            ...
            'compromisso': {
                'id': novo_compromisso.id,
                'cliente_nome': novo_compromisso.cliente_nome,
                'data_servico': novo_compromisso.data_servico.strftime('%Y-%m-%d'),
                'servico': novo_compromisso.servico,
                'cliente_telefone': novo_compromisso.cliente_telefone or '',
                'observacoes': novo_compromisso.observacoes or ''
            }
        })
        
    except Exception as e:
        db.session.rollback()
        app.logger.error(f'Erro ao agendar: {str(e)}', exc_info=True)
        return jsonify({'success': False, 'message': f'Erro ao agendar: {str(e)}'}), 500
...
def api_compromissos():
    try:
        ...
        compromissos_data = [{
            ...
            'data_servico': c.data_servico.strftime('%Y-%m-%d'),
            'servico': c.servico,
        ...
        app.logger.error(f'Erro ao carregar compromissos: {str(e)}', exc_info=True)
        return jsonify({'success': False, 'message': f'Erro ao carregar compromissos: {str(e)}'}), 500

@app.route('/api/excluir/<int:compromisso_id>', methods=['DELETE'])
def api_excluir_compromisso(compromisso_id):
    try:
        compromisso = Orcamento.query.get_or_404(compromisso_id)
        
        cliente_nome = compromisso.cliente_nome
        db.session.delete(compromisso)
        db.session.commit()
        
        return jsonify({
            'success': True,
            'message': f'Agendamento de {cliente_nome} excluído com sucesso!'
        })
        
    except Exception as e:
        db.session.rollback()
        app.logger.error(f'Erro ao excluir compromisso {compromisso_id}: {str(e)}', exc_info=True)
        return jsonify({'success': False, 'message': f'Erro ao excluir: {str(e)}'}), 500

@app.route('/api/excluir-multiplos', methods=['DELETE'])
def api_excluir_multiplos():
    try:
        data = request.get_json()
        ids = data.get('ids', [])
        
        if not ids:
            return jsonify({'success': False, 'message': 'Nenhum compromisso selecionado'}), 400
        
        compromissos = Orcamento.query.filter(Orcamento.id.in_(ids)).all()
        
        if not compromissos:
            return jsonify({'success': False, 'message': 'Compromissos não encontrados'}), 404
        
        count = len(compromissos)
        ...
        return jsonify({
            'success': True,
            'message': f'{count} agendamento(s) excluído(s) com sucesso!'
        })
        
    except Exception as e:
        db.session.rollback()
        app.logger.error(f'Erro ao excluir múltiplos compromissos: {str(e)}', exc_info=True)
        return jsonify({'success': False, 'message': f'Erro ao excluir: {str(e)}'}), 500
